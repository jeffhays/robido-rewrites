...
4. Copy the config/initializers/secret_token.rb.example file to config/intializers/secret_token.rb and edit the contents:
    mv config/initializers/secret_token.rb.example config/initializers/secret_token.rb

5. Edit the secret_token.rb file and place your newly generated secret token in there
    rake secret

6. Copy the output of the above command and place it on line 7 of config/initializers/secret_token.rb accordingly

7. Save your changes, navigate to your app's document root, and start the rails server
    rails server

8. Browse to http://localhost:3000 and upload a PCAP file!
